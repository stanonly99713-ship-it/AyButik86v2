// ВРЕМЕННЫЕ данные для этапа P0 — витрина верстается и деплоится раньше,
// чем появляется база данных (P1) и реальные фото мамы (P3). Формы типов
// совпадают с будущей схемой Drizze, поэтому на P1 этот файл просто
// перестанет импортироваться, а не переписываются компоненты.
//
